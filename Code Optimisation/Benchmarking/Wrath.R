...
# Author: Gareth Burns
# Date: 08/07/2023
# This is the Greed.R script with the improvement of using an sapply in place of
# for loop to avoid building the results object
...
results <- data.frame(sim = numeric(length(sims)),
                      score = character(length(sims)))

# Create Deck of cards
deckValues <- rep(1:13, times = 4)

results <- sapply(sims, function(sim) {
